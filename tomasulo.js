class Tomasulo {
  ...
  raw(instruction, index) {
    ...
    for (i = 0; i < index; i++) {
      ...
      if (
        (instruction[2] == write_at || instruction[3] == write_at) &&
  ...
  war(instruction, index) {
    ...
    for (i = 0; i < index; i++) {
      ...
      // Se há leitura pós escrita, e a instrução anterior ainda não foi finalizada
      if (
        (instruction[1] == read || instruction[1] == read_b) &&
        this.instructions_state[i] != "commit"
      ) {
        dependency = true; // Há dependencia de dados
      }
  ...
  hasDependency(inst, i) {
    var dependency =
      this.raw(inst, i) || this.war(inst, i) || this.waw(inst, i)
        ? true
        : false;
    return dependency;
  }
  ...
  next() {
    ...
    while (i < this.instr_ammount && flag) {
      // Loop que percorre todas as intstruções
      // Verificar se há dependencia de dados
      var dependency =
        i != 0 ? this.hasDependency(this.instructions[i], i) : false; // Na primeira instrução não há necescidade de verificar dependência
      ...
      switch (this.instructions_state[i]) {
        case "awaiting": // Se a instrução estiver aguardando liberação de FU ou dependencia de dados
          ...
          if (!dependency && found_fu) {
            // Se não há dependencia de dados, e há FU livre
        ...
        case "issue":
          if (dependency) {
            // Se há dependência, a instrução deverá aguardar
            debugger;
